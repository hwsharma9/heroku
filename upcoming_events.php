<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta name="description" content="To achieve excellent standards of quality education by keeping pace with rapidly changing technologies & create computer professionals of global standards with capabilities of accepting new challenges.">
    <meta name="keywords" content="SCSIT, INDORE, DAVV, COMPUTER SCIENCE, COLLEGE">
    <meta name="author" content="lishugupta">
    <title>Scsit Davv</title>
    <link rel="icon" href="./images/logo.png" type="image/png" sizes="16x16">
    <!-- SCRIPTS START   -->
    <?php include './includes/scriptincludes.php';?>
    <!-- SCRIPTS END -->
</head>

<body>

    <div class="root">
        <!-- HEADER START   -->
        <?php include './includes/header.php';?>
        <!-- HEADER END -->

        <div class="page-title"></div>

        <!-- main content area -->
        <div class="primary-container">
            <div class="main-content-area" id="main-content-area">
                <div class="content-left padding-border">
                    <!-- ADD NEW CONTENT DOWN HERE -->
                    <h1>Events</h1>
                    <h2>Cultural Events</h2>
                    <hr>
                    <ul type="disc" style="margin-left: 20px;">
                        <li>
                            A SPIC MACAY Program (VIRASAT 2019 Series) on 19th September, 2019
                            <a class="content-link" href="assets/pdf/Report SPIC MACAY2019.pdf"> Click Here.. </a>
                        </li>
                        <br />

                        <li>
                            Annual Function: Genesis 2019
                            <a class="content-link" href="assets/pdf/Report GENESIS  2019.pdf"> Click Here.. </a>
                        </li>
                        <br />
                        <li>
                            Annual Function Genesis 2018
                            <a class="content-link" href="assets/pdf/Report GENESIS 2018.pdf"> Click Here.. </a>
                        </li>
                        <br />
                        <li>
                            Annual Function Genesis 2017
                            <a class="content-link" href="assets/pdf/Report_Genesis2K17.pdf"> Click Here.. </a>
                        </li>
                        <br />

                        <li>
                            SPIC MACAY Programme by Pandit Ajay Pohankar, world renowned
                            Vocalist (Hindustani Music), Shri Ramendra Singh Solanki on Tabla,
                            and Dr. Vivek Bansod on Harmonium, January 20, 2016.
                        </li>
                        <br />
                    </ul>
                    <h2>Technical Events</h2>
                    <hr>
                    <ul type="disc" style="margin-left: 20px;">
                        <li>
                            Report of Faculty Development Programme Introduction to
                            Programming: A Pedagogical Approach 17-22 June 2019
                            <a class="content-link" href="assets/pdf/FDP_Report_Programmming_17_22_June_2019.pdf">
                                Click Here..
                            </a>
                        </li>
                        <br />

                        <li>
                            Group Reading (Samuhik Vachan) of Literature Related to Shri
                            Mahatma Gandhiji for Celebrating 150th Birth Anniversary, 2nd Oct,
                            2019.
                        </li>
                        <br />

                        <li>
                            Faculty Development Programme on Introduction to Programming: A
                            Pedagogical Approach in coordination with Electronics and ICT
                            Academy PDPM IIITDM, Jabalpur, from 17-22 June 2019
                        </li>
                        <br />

                        <li>
                            Three Weeks Technical Workshop on AWS Certified Solution
                            Architect- Associate from 12th March, 2019 to 13th April, 2019.
                            <a class="content-link" href="assets/pdf/AWS Solution Architect-Workshop-Contents.pdf">
                                Click Here..
                            </a>
                        </li>
                        <br />

                        <li>
                            Faculty Development Programme on Internet of Things (IoT) in
                            coordination with Electronics and ICT Academy PDPM IIITDM,
                            Jabalpur, from June 18-22, 2018.
                        </li>
                        <br />
                        <li>
                            Faculty Development Programme on ANN and Deep Learning in
                            coordination with Electronics and ICT Academy PDPM IIITDM,
                            Jabalpur, from June 11-15, 2018.
                        </li>
                        <br />
                        <li>
                            Awareness Programme on Outcome Based Education & Accreditation,
                            8th June, 2018.
                        </li>
                        <br />
                        <li>
                            Two days workshop on Algorithm Design & Program Optimization
                            Techniques from 14th April, 2018 to 15th April, 2018.
                        </li>
                        <br />

                        <li>
                            Seminar on IT Security & Ethical Hacking by Jafar Hasan (APPIN
                            Technology Lab, Indore), February 11, 2018.
                            <a class="content-link" href="assets/pdf/Ethical hacking report.pdf"> Click Here.. </a>
                        </li>
                        <br />

                        <li>
                            Faculty Development Programme on Machine Learning Basics &
                            Emerging Trends in coordination with Electronics and ICT Academy
                            PDPM IIITDM, Jabalpur, from September 1-6, 2017.
                        </li>
                        <br />
                        <li>
                            National Workshop on Quality Technical Education jointly organised
                            by Institute of Engineering & Technology and School of Computer
                            Science & IT, DAVV, 9th June, 2017
                        </li>
                        <br />
                        <li>
                            Three Weeks Workshop on BIG DATA ANALYTICS USING HADOOP from 14th
                            June 2017 to 4th July 2017
                            <a class="content-link" href="assets/pdf/Bigdata using Hadoop Report.pdf"> Click Here.. </a>
                        </li>
                        <br />

                        <li>
                            Workshop/ Training program on Internet of Things ( IoT) 17th to
                            27th May 2017.
                            <a class="content-link" href="assets/pdf/Report IOT.pdf"> Click Here.. </a>
                        </li>
                        <br />

                        <li>
                            Workshop for Industry Institute Interaction - 2017 (10th March
                            2017).
                        </li>
                        <br />
                        <li>
                            Expert lecture on "Moving Object Segmentation and Multi-View Human
                            Activity Recognition for Video Surveillance" by Dr. Alok Kushwaha,
                            Ph.D. (IIT (BHU), Varanasi) on 19th January, 2017.
                        </li>
                        <br />

                        <li>
                            Seminar on IT Security & Ethical Hacking by Jafar Hasan (APPIN
                            Technology Lab, Indore), February 11, 2016.
                        </li>
                        <br />
                        <li>
                            Lecture on Campus to Corporate by Mr. Sandeep Atre ( CH Edge
                            Makers), February 3rd, 2016.
                        </li>
                        <br />
                    </ul>
                    <h2>Others</h2>
                    <hr>
                    <ul type="disc" style="margin-left: 20px;">
                        <li>
                            Parents-Teachers Meet for BCA V Semester, 12th Oct, 2019.
                            <a href="PTM-BCA-V report.pdf"> Click Here.. </a>
                        </li>
                        <br />
                    </ul>
                </div>

                <div class="sidebar content-right">
                    <div class="sidebar-content" id="sidebar">
                        <!-- SIDEBAR LINKS START   -->
                        <?php include './includes/sidebarlinks.php';?>
                        <!-- SIDEBAR LINKS END -->

                    </div>
                </div>
            </div>
        </div>


        <!-- FOOTER START   -->
        <?php include './includes/footer.php';?>
        <!-- FOOTER END -->

    </div>

</body>

</html>